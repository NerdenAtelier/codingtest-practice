...
def solution(clothes) :
    closet_dict = collections.defaultdict(list)

    answer = 1

    # 주어진 배열의 값을 확인하여, 딕셔너리에 카테고리를 key로 하는 value list에 삽입
    for i in clothes :
        closet_dict[i[1]].append(i[0])

    # 각 카테고리별 의상 숫자에 1을 더해 모두 곱해 줌
    for v in closet_dict.values() :
        answer *= (len(v)+1)
