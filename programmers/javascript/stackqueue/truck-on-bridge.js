...
function solution(bridge_length, weight, truck_weights) {
  ...
  let onbridge_weight = 0;
  const arrived_trucks = [];
  numsOfTrucks = truck_weights.length();

  while (arrived_trucks.length < numsOfTrucks) {
    let a = bridge.shift();
    time += 1;
    if (a != 0) {
      arrived_trucks.push(a);
      onbridge_weight -= a;
    }
    if (
      truck_weights.length > 0 &&
      weight - onbridge_weight >= truck_weights[0]
    ) {
      b = truck_weights.shift();
      bridge.push(b);
      onbridge_weight += b;
    } else {
      bridge.push(0);
